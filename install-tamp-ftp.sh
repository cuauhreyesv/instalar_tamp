...
# Script principal de inicio SIN 'tamp start'
cat > ~/iniciarservicios << 'AUTO_EOF'
#!/data/data/com.termux/files/usr/bin/bash
# ============================================
# AUTO-INICIO MEJORADO v3.1
# VERSIÓN MANUAL - SIN 'tamp start' QUE ABRE NAVEGADOR
# ============================================
echo ""
echo "🔧 SISTEMA DE AUTO-INICIO MEJORADO v3.1"
...
# Función para iniciar servicio MANUALMENTE
start_service_manual() {
    ...
    case "$service_name" in
        "Apache Web Server")
            # Iniciar Apache manualmente - SIN 'tamp start'
            if [ -f ~/tamp-cuauh/apache/bin/httpd ] && [ -f ~/tamp-cuauh/apache/conf/httpd.conf ]; then
                ~/tamp-cuauh/apache/bin/httpd -f ~/tamp-cuauh/apache/conf/httpd.conf > /dev/null 2>&1 &
                sleep 3
            else
                echo "⚠️  No se encontraron archivos de Apache"
                return 1
            fi
            ;;
        "MySQL Database")
...
# 1. INICIAR APACHE MANUALMENTE (NO usar tamp start)
...
# 5. MOSTRAR RESUMEN
echo ""
echo "📊 RESUMEN DE SERVICIOS (MÉTODO MANUAL):"
echo "════════════════════════════════════════"
...
echo "════════════════════════════════════════"
echo "pkill -f httpd              # Detener Apache"
...
echo ""
echo "✅ Todos los servicios iniciados MANUALMENTE"
echo "   • Método: Sin 'tamp start'"
echo "   • Navegador: NO se abrirá automáticamente"
AUTO_EOF
...
# ============================================
# SISTEMA DE AUTO-INICIO MEJORADO v3.1
...
if [ -f ~/iniciarservicios ]; then
    ...
    if [[ $- == *i* ]]; then
        ...
        if [ $SERVICES_NEEDED -eq 1 ]; then
            ...
            echo "   • Apache Web Server (manual)"
            echo "   • MySQL Database (manual)"
...
echo "   • NAVEGADOR: COMPLETAMENTE DESACTIVADO"
echo "   • No usa 'tamp start' (usa métodos manuales)"
...
# 9. EJECUTAR SERVICIOS POR PRIMERA VEZ (MÉTODO MANUAL - SIN NAVEGADOR)
...
# 1. Iniciar Apache manualmente
echo "🔄 Iniciando Apache Web Server (manual)..."
if [ -f ~/tamp-cuauh/apache/bin/httpd ] && [ -f ~/tamp-cuauh/apache/conf/httpd.conf ]; then
    ~/tamp-cuauh/apache/bin/httpd -f ~/tamp-cuauh/apache/conf/httpd.conf &
    APACHE_PID=$!
    sleep 3
    
    if pgrep -f "httpd" > /dev/null; then
        echo "✅ Apache Web Server: Iniciado correctamente"
    else
        echo "⚠️  Apache Web Server: Tuvo problemas al iniciar"
    fi
else
    echo "❌ No se encontraron archivos de Apache"
fi
...
echo ""
echo "✅ TAMP Web Server (Apache + MySQL + PHP) v3.1"
