...
# 5. MOSTRAR RESUMEN
echo ""
echo "📊 RESUMEN DE SERVICIOS (MÉTODO MANUAL CORREGIDO):"
echo "════════════════════════════════════════"
...
echo "════════════════════════════════════════"
echo "apachectl stop            # Detener Apache CORRECTO"
...
#!/data/data/com.termux/files/usr/bin/bash
# ============================================
# VERIFICADOR DE ESTADO DE SERVICIOS - CORREGIDO
# ============================================
echo ""
echo "🔍 VERIFICANDO ESTADO DE SERVICIOS (CORREGIDO)"
echo "════════════════════════════════════════"
...
if apachectl status 2>/dev/null | grep -q "running"; then
    echo "✅ Apache Web Server: 🟢 ACTIVO (puerto 8080)"
else
    echo "❌ Apache Web Server: 🔴 INACTIVO"
    echo "   • Solución: Ejecuta 'apachectl start'"
fi
...
echo "📁 DIRECTORIO WEB:"
if [ -d "/sdcard/htdocs" ]; then
    echo "✅ /sdcard/htdocs: Existe"
    echo "   • Archivos: $(ls /sdcard/htdocs | wc -l) archivos"
else
    echo "❌ /sdcard/htdocs: No existe"
    echo "   • Solución: Ejecuta 'mkdir -p /sdcard/htdocs'"
fi
...
if [ -f ~/iniciarservicios ]; then
    ...
    if [[ $- == *i* ]]; then
        ...
        if [ $SERVICES_NEEDED -eq 1 ]; then
            echo ""
            echo "🔄 Iniciando servicios automáticamente (CORREGIDO)..."
...
echo ""
echo "🔧 COMANDOS NUEVOS - CORREGIDOS:"
echo "════════════════════════════════════════"
echo "~/check_services           # Verificar estado"
echo "~/iniciarservicios         # Reiniciar servicios"
echo "~/tamp-ftp-2221           # FTP específico"
echo "apachectl start           # Iniciar Apache (CORREGIDO)"
echo "apachectl status          # Estado de Apache (CORREGIDO)"
echo ""
...
echo "🔍 TEST FINAL:"
echo "════════════════════════════════════════"
echo "1. Prueba Apache (CORREGIDO):"
echo "   • Ejecuta: ~/check_services"
echo "   • Deberías ver: 'Apache Web Server: ACTIVO'"
echo ""
echo "2. Prueba MySQL:"
echo "   • http://localhost:8080/test-mysql-terminal1.php"
echo ""
echo "3. phpMyAdmin (REPARADO):"
echo "   • http://localhost:8080/phpmyadmin/"
echo "   • User: terminal1 / Pass: Master01"

echo ""
echo "🛠️  SOLUCIÓN DE PROBLEMAS - ACTUALIZADO:"
echo "════════════════════════════════════════"
echo "Si Apache no inicia:"
echo "1. Verifica instalación: apachectl -v"
echo "2. Inicia manualmente: apachectl start"
echo "3. Verifica estado: apachectl status"
echo "4. Revisa logs: cat ~/.startup.log"
echo ""
...
echo "apachectl stop          # Detener Apache (CORREGIDO)"
echo "pkill -f mysqld         # Detener MySQL"
echo "pkill -f pyftpdlib      # Detener FTP"
...
echo "════════════════════════════════════════"
echo "• Apache ahora usa 'apachectl' en lugar de rutas incorrectas"
echo "• Esto corrige el problema de Apache que no iniciaba"
echo "• El navegador NO se abrirá automáticamente"
echo "• Para acceder: Abre tu navegador y ve a http://localhost:8080"