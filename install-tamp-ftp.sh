...
show_banner() {
    echo -e "${CYAN}"
    echo "╔════════════════════════════════════════════════════════════╗"
    echo "║          🚀 TAMP SERVER INSTALLER v3.0 🚀                 ║"
...
# ============================================
# PARCHES PARA DESACTIVAR NAVEGADOR
# ============================================
print_info "Aplicando parches para desactivar navegador automático..."

# 1. Verificar si hay scripts que abran navegador en el repositorio
if [ -f ~/tamp-cuauh/setup ]; then
    # Remover cualquier comando que abra navegador
    sed -i 's/am start.*//g' ~/tamp-cuauh/setup 2>/dev/null || true
    sed -i 's/xdg-open.*//g' ~/tamp-cuauh/setup 2>/dev/null || true
    sed -i 's/open .*//g' ~/tamp-cuauh/setup 2>/dev/null || true
    sed -i 's/start .*//g' ~/tamp-cuauh/setup 2>/dev/null || true
    print_success "Parche aplicado a setup"
fi

# 2. Crear variable de entorno para desactivar navegador
export NO_BROWSER_AUTO_OPEN=1
echo "export NO_BROWSER_AUTO_OPEN=1" >> ~/.bashrc
echo "export NO_BROWSER_AUTO_OPEN=1" >> ~/.profile

# 3. Verificar otros scripts en el repositorio
find ~/tamp-cuauh -type f -name "*.sh" -o -name "*.bash" | while read file; do
    if grep -q -E "(am start|xdg-open|open http|start http)" "$file" 2>/dev/null; then
        echo "⚠️  Encontrado comando de navegador en: $file"
        # Comentar las líneas que abren navegador
        sed -i 's/am start/# am start/g' "$file" 2>/dev/null || true
        sed -i 's/xdg-open/# xdg-open/g' "$file" 2>/dev/null || true
    fi
done

print_info "Ejecutando instalador TAMP (sin navegador)..."
cd ~/tamp-cuauh

# Ejecutar setup de manera segura (sin navegador)
bash setup 2>&1 | grep -v -E "(am start|xdg-open|open|starting browser)" | tee ~/tamp-install.log

if [ $? -eq 0 ] || grep -q "success\|complete\|done" ~/tamp-install.log 2>/dev/null; then
    print_success "TAMP instalado correctamente desde cuauhreyesv/tamp"
else
    print_warning "Hubo algunos mensajes durante la instalación, revisando log..."
    tail -20 ~/tamp-install.log
    print_info "Continuando con la configuración..."
fi
...
# 5. CREAR SISTEMA DE AUTO-INICIO MEJORADO
...
# ============================================
echo ""
echo "🔧 SISTEMA DE AUTO-INICIO MEJORADO v3.0"
...
# Función para iniciar servicio con verificación
start_service() {
    ...
    if is_service_running "$process_name"; then
        ...
        # Desactivar cualquier apertura de navegador
        NO_BROWSER_AUTO_OPEN=1 eval "$start_command" > /dev/null 2>&1 &
        sleep 3
...
# 1. INICIAR TAMP (WEB SERVER) - SIN NAVEGADOR
echo "🔄 Iniciando TAMP Web Server (sin navegador)..."
NO_BROWSER_AUTO_OPEN=1 tamp start > /dev/null 2>&1 &
sleep 5

if is_service_running "httpd"; then
    echo "✅ TAMP Web Server: Iniciado correctamente"
else
    echo "⚠️  TAMP Web Server: Tuvo problemas al iniciar"
fi
...
echo "════════════════════════════════════════"
echo "1. Abre tu navegador web manualmente"
...
echo ""
echo "✅ Todos los servicios verificados/iniciados"
echo "   Navegador: NO se abrió automáticamente"
AUTO_EOF

chmod +x ~/iniciarservicios

# 6. CREAR SCRIPT DE VERIFICACIÓN DE ESTADO
cat > ~/check_services << 'CHECK_EOF'
#!/data/data/com.termux/files/usr/bin/bash
# ============================================
# VERIFICADOR DE ESTADO DE SERVICIOS
# ============================================
echo ""
...
echo ""
echo "🗄️  USUARIO MYSQL:"
if mysql -u terminal1 -pMaster01 -e "SELECT '✅ terminal1 funciona' as Status;" 2>/dev/null; then
    echo "✅ terminal1: Configurado y accesible"
else
    echo "⚠️  terminal1: No accesible o no existe"
fi
...
if [ -n "$IP" ]; then
    ...
    echo "   • FTP: ftp://$IP:2221"
    echo "   • MySQL: $IP:3306"
else
    echo "   • Solo acceso local"
fi
...
print_header "CONFIGURANDO AUTO-INICIO MEJORADO"
...
# Agregar nuevo sistema de autoinicio
cat >> ~/.bashrc << 'BASHRC_EOF'
# ============================================
# SISTEMA DE AUTO-INICIO MEJORADO
...
if [ -f ~/iniciarservicios ]; then
    ...
    if [[ $- == *i* ]]; then
        ...
        if [ $SERVICES_NEEDED -eq 1 ]; then
            ...
            (NO_BROWSER_AUTO_OPEN=1 bash ~/iniciarservicios > ~/.startup.log 2>&1 &)
        else
            echo ""
            echo "✅ Todos los servicios están activos"
            echo "   • Web: http://localhost:8080"
            echo "   • FTP: puerto 2221"
            echo "   • MySQL: terminal1/Master01"
        fi
    fi
fi
BASHRC_EOF
...
print_success "Sistema de autoinicio mejorado configurado"
print_info "El sistema ahora:"
echo "   • Verifica servicios antes de iniciar"
echo "   • No usa archivos de bloqueo"
echo "   • Funciona en cada reinicio"
echo "   • NAVEGADOR: COMPLETAMENTE DESACTIVADO"
echo "   • Variable NO_BROWSER_AUTO_OPEN establecida"
...
TERMUX_BOOT_EOF
    ...
    print_success "Termux-boot configurado"
...
# 9. EJECUTAR SERVICIOS POR PRIMERA VEZ (SIN NAVEGADOR)
...
echo "   • NAVEGADOR: NO se abrirá en ningún momento"
echo "   • Usando NO_BROWSER_AUTO_OPEN=1"

# Ejecutar auto-inicio SIN navegador
NO_BROWSER_AUTO_OPEN=1 bash ~/iniciarservicios > ~/first_start.log 2>&1 &

# Esperar a que todo inicie
sleep 10

# 10. VERIFICACIÓN COMPLETA
print_header "VERIFICACIÓN DE ESTADO MEJORADA"
echo ""
...
🚀 COMANDOS DE CONTROL:
   ...
   ▸ ~/iniciarservicios  # Iniciar todos los servicios
...
# 14. MOSTRAR RESUMEN FINAL (SIN ABRIR NAVEGADOR)
print_header "🎉 INSTALACIÓN COMPLETADA CON ÉXITO"
echo ""
echo "🏆 ¡FELICITACIONES! 🏆"
echo "Has instalado exitosamente:"
echo ""
echo "✅ TAMP Web Server (Apache + MySQL + PHP) v3.0"
...
echo "   • Problema anterior: Solo funcionaba una vez"
echo "   • Solución: Verificación inteligente"
echo "   • Resultado: Funciona en cada reinicio"
echo ""
echo "✅ NAVEGADOR"
...
echo "tamp start                 # Solo web server"
...
echo "3. Deberías ver: '🔄 Iniciando servicios automáticamente...'"
echo "4. Navegador: NO se abrirá automáticamente"
echo "5. Para acceder manualmente:"
...
echo "   • User: terminal1 / Pass: Master01"
echo ""
echo "3. Verifica estado:"
echo "   • ~/check_services"

echo ""
echo "🛠️  SOLUCIÓN DE PROBLEMAS:"
echo "════════════════════════════════════════"
echo "Si el auto-inicio no funciona:"
echo "1. Verifica: ~/check_services"
echo "2. Inicia manualmente: ~/iniciarservicios"
echo "3. Revisa logs: cat ~/.startup.log"
echo ""
echo "Si phpMyAdmin aún falla:"
echo "1. Diagnóstico: http://localhost:8080/phpmyadmin-diag.php"
echo "2. Configuración: ~/tamp-cuauh/apache/htdocs/phpmyadmin/config.inc.php"
