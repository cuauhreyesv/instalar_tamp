...
# 12. CONFIGURAR AUTO-INICIO EN ANDROID (EJECUTA TERMUX AL ARRANCAR)
print_header "CONFIGURANDO AUTO-INICIO EN ANDROID"
# Crear servicio de inicio Android
cat > /sdcard/startup-tamp.sh << 'STARTUP_EOF'
#!/system/bin/sh
# ============================================
# SERVICIO DE INICIO ANDROID - EJECUTA TERMUX
# Termux ya tiene auto-inicio configurado en .bashrc
# ============================================
LOG_FILE="/sdcard/android-startup.log"
echo "[$(date)] ===== INICIANDO TERMUX AL ARRANQUE =====" > $LOG_FILE
# Esperar condiciones del sistema
wait_for_system() {
    # 1. Esperar arranque completo
    while [ "$(getprop sys.boot_completed)" != "1" ]; do
        sleep 3
    done
    echo "[$(date)] ✅ Sistema Android listo" >> $LOG_FILE
    # 2. Esperar almacenamiento
    while [ ! -d "/sdcard" ]; do
        sleep 3
    done
    echo "[$(date)] ✅ Almacenamiento montado" >> $LOG_FILE
    # 3. Esperar un poco más para asegurar
    sleep 15
}
# Función principal - Iniciar Termux
launch_termux() {
    echo "[$(date)] 🚀 Lanzando Termux..." >> $LOG_FILE
    # Método 1: Ejecutar comando directamente en Termux
    # Esto requiere que Termux tenga el permiso de "ejecutar comandos"
    am start -n com.termux/.app.TermuxActivity \
        --es com.termux.RUN_COMMAND "bash ~/auto-start-all" \
        --ez com.termux.RUN_COMMAND_BACKGROUND "true" \
        --ez com.termux.RUN_COMMAND_WAKE_LOCK "true"
    # Método 2 (alternativo): Solo abrir Termux
    # am start -n com.termux/.app.TermuxActivity
    echo "[$(date)] ✅ Comando enviado a Termux" >> $LOG_FILE
    # Dar tiempo a Termux para iniciar
    sleep 10
}
# Verificar si Termux se ejecutó
verify_execution() {
    echo "[$(date)] 🔍 Verificando procesos..." >> $LOG_FILE
    # Esperar a que Termux procese el comando
    sleep 20
    # Verificar si los servicios de TAMP están corriendo
    if pgrep -f "httpd" > /dev/null; then
        echo "[$(date)] ✅ Apache funcionando (8080)" >> $LOG_FILE
    else
        echo "[$(date)] ⚠️  Apache NO detectado" >> $LOG_FILE
    fi
    if pgrep -f "pyftpdlib" > /dev/null; then
        echo "[$(date)] ✅ FTP funcionando (2221)" >> $LOG_FILE
    else
        echo "[$(date)] ⚠️  FTP NO detectado" >> $LOG_FILE
    fi
}
# Función para crear notificación
send_notification() {
    # Intentar crear notificación simple
    su -c "cmd notification post -t 'TAMP Server' 'Tag' 'Servidores listos: Web:8080 | FTP:2221'" 2>/dev/null || \
    echo "[$(date)] ℹ️  Notificación no enviada (permisos)" >> $LOG_FILE
}
# ===== EJECUCIÓN PRINCIPAL =====
wait_for_system
launch_termux
...
STARTUP_EOF
chmod 755 /sdcard/startup-tamp.sh
print_success "Script de inicio Android creado: /sdcard/startup-tamp.sh"
print_info "📱 Para activar el auto-inicio en Android, necesitas:"
print_info "1. Instalar Tasker, Automate o Macrodroid"
print_info "2. Configurar un perfil que ejecute al arrancar:"
print_info "   sh /sdcard/startup-tamp.sh"
print_info "3. Conceder permisos necesarios a la app"
...
🔄 AUTO-INICIO:
   • Termux: Se inicia al abrir Termux (~/.bashrc)
   • Android: Script listo en /sdcard/startup-tamp.sh
   • Configurar con Tasker/Automate para auto-arranque
...
echo "   • Contraseña: android"
echo ""
echo "✅ Sistema de Auto-Inicio Android"
echo "   • Script: /sdcard/startup-tamp.sh"
echo "   • Ejecuta Termux automáticamente al arrancar"
echo ""
# Mostrar IP actual
IP=$(ifconfig 2>/dev/null | grep -oE 'inet ([0-9]{1,3}\.){3}[0-9]{1,3}' | grep -v '127.0.0.1' | head -1 | awk '{print $2}')
if [ -n "$IP" ]; then
...
echo "• /sdcard/startup-tamp.sh # Inicio Android"
...
print_warning "⚠️  Para auto-inicio en Android, configura manualmente"
echo "   Tasker/Automate con: sh /sdcard/startup-tamp.sh"
...
echo ""
echo "Para auto-inicio en Android:"
echo "1. Instalar Tasker, Automate o Macrodroid"
echo "2. Crear perfil 'Al arrancar dispositivo'"
echo "3. Añadir acción: Ejecutar Shell"
echo "4. Comando: sh /sdcard/startup-tamp.sh"
echo ""
...
echo "   Auto-inicio configurado para Termux y Android"
echo "────────────────────────────────────────────"